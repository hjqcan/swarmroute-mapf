...
`.csproj` 引用了 Map、PathPlanning、TrafficControl 的契约(接口)+ domain 程序集,加上 Kernel。它**不**引用 Liveness 上下文:Coordination 从不从这个回路发起活性处理——授予时的环**预防**位于 TrafficControl 的 `ReservationTable.TryGrant` 内部(由它查询 Liveness 的 `IWouldCloseCycleDetector`),而物理僵局**解决**位于 Simulation 执行器(`FleetLoopDriver`,由它查询 `ILivenessPolicy`)。它只引入 `Microsoft.Extensions.*` 抽象(Hosting、DI、Logging、Options)——不依赖任何具体基础设施。
...
**它与单个周期的区别:** 回路增加了*时间、一个目标源,以及故障隔离*。周期是 `(roadmap, goals, view, clock)` → `CycleReport` 的纯函数;回路提供*何时*(看门狗 tick)、*做什么*(实时目标簿)以及*保活*(吞异常并继续)。活性处理**不**属于这个内层回路:等待环**预防**在授予时于 TrafficControl 内部发生(Liveness 的 `IWouldCloseCycleDetector`,由 `ReservationTable.TryGrant` 查询),而物理僵局**解决**是 Simulation 执行器的职责(`ILivenessPolicy`,由 `FleetLoopDriver` 查询)。Coordination 只做 规划 → 预约 → 剪枝重规划(`:28-30`)。
...
闭环端到端运行(执行器驱动多个 tick 直至完成)由针对 `FleetLoopDriver`(Simulation 上下文)的 `ClosedLoopIntegrationTests.cs` 覆盖。授予时的等待环预防——本回路的预约与 Liveness 之间唯一的耦合——由 `CyclePreventionIntegrationTests.cs` / `WouldCloseCycleDetectorTests.cs`(Liveness 的 `RagCycleDetector` 筛查 `TryGrant`)单独覆盖;已不再有反应式死锁扫描测试,因为那条流程已被移除。
