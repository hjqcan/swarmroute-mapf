...
| **v2** 優先級 + RHCR + 避免式預防 | 下一階段 | 優先級 SIPP + PIBT；滾動時窗；`WouldCloseCycle` 接進 `TryReserve` | 高吞吐、延遲有界、**構造性 liveness** |
| **v3** CBS/CCBS + SIPPwRT | 下一階段 | 壅塞 Zone 局部 CBS；連續時間 + 運動學 | 最高密度、平滑時間精確運動 |
