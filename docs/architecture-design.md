...
| **v3** PIBT + 局部 CBS + 連續時間 SIPPwRT + host-seam（壅塞區聯合求解，全三支柱交付） | ✅ 已交付 | **三大支柱全交付**：① 執行層 **zone-local PIBT**（opt-in `UsePibt`）；② **局部 CBS/CCBS**（opt-in `UseCbs`，`CbsLocalSolver` 在 `PathPlanning.Domain`、重用 `SippPathPlanner` 當低階、經 `IFleetCoordinationCycle.PlanClusterAsync` 觸發+原子預約）；③ **連續時間 SIPPwRT**（real-ms 軌跡 replay）。共用 `StuckClusterDetector` 偵測物理僵局簇 → 釋放預約 → PIBT 逐 tick 聯合步進 或 CBS 聯合求解 → 回 prioritized-SIPP。兩者是互斥的同層 cluster owner（同一物理僵局簇不可雙開）；CBS 只允許 SIPP/schedule-faithful，並沿用 RHCR horizon 與 executor physical blockers。**並含 host-seam**：`IJointStepPlanner` + `ReservationTable.TryGrantJointStep`（原子聯合提交）+ 自治 `FleetCoordinationLoop` 聯合求解器（`JointResolver=Cbs/Pibt`，經 `IFleetCoordinationCycle.ResolveStandoffsAsync`）。皆預設關、executor-triggered。**接縫誠實校正**：TrafficControl 寫接縫為**加法擴充**（新增 `TryGrantJointStep`、不改既有契約）；惟 host-loop 聯合求解使 **`Coordination.Application` 編譯期依賴 `Liveness.Domain`**（壅塞簇偵測 `StuckClusterDetector`/`HopDistances` + `IJointStepPlanner` 港與 `PibtJointStepPlanner` 實作），此跨層耦合屬實 —— 非「零接縫變動」。更乾淨的歸屬（host-loop 聯合求解整體移入 Liveness，或 port 移回 Coordination）為待議重構 | 高密度物理僵局收斂（多 seed 由 DidNotConverge → Completed/更多抵達，全程無碰撞）；CBS 為完整局部解法（單測證明攻下 PIBT 攻不下的靜態案例）；SIPPwRT 連續時間軌跡；host-seam 以表為權威封 cross-tick 健全性、帶 PIBT/CBS 進 autonomous host loop |
...
> v2/v3 決策記錄（AA-SIPP(m)/h<w 否決；**v3 PIBT + 局部 CBS 採 executor-anchored、僅 sim/執行層**；CBS 重用 SIPP 低階、動態迴圈不主宰 PIBT；連續時間 SIPPwRT/CCBS + PIBT host-seam 已於 v3 交付；TrafficControl 讓路為 ordinal-id 非真 priority；WouldCloseCycle 在執行層 sim 中 inert）見 DDD 開發計劃 §「v2/v3 決策記錄」。
