...
**現況診斷（業界視角）**：現有引擎是「**整條路徑空間上鎖 + 剪枝 Dijkstra + 事後 RAG 死鎖偵測**」的第一代 AGV 交管——`GraphMap.GeneratePath()` 規劃完直接把整條路線的站點/路段/區塊鎖給一台車，**沒有時間維度**，因此兩台車不能錯時共用走廊、吞吐受限、且天生易死鎖。業界已演進到**時空預約表 / 安全區間規劃（SIPP）**、**滾動時窗終身重規劃（RHCR）**、**優先級規劃 / PIBT / CBS**；參考車隊管理架構為 **OpenTCS**（Router＝路徑規劃、Scheduler＝資源配置/交管、Dispatcher＝派工）。本架構把現有引擎當 **v0 基線**移植進乾淨架構，再沿 v1→v3 演進。
...
3. **DDD 基底**：內嵌 grukirbs 的 NetDevPack fork（retarget net10.0）。
4. **首個里程碑**：腳手架 + v0 基線；v1–v3 為路線圖。
...
│  └─ vendor/NetDevPack/                   # 內嵌的 NetDevPack fork（retarget net10.0）
│
...
| 版本 | 變更 | 買到 |
|---|---|---|
| **v0**（首里程碑） | 移植「剪枝 Dijkstra + 整路上鎖 + RAG 偵測 + 最小恢復」 | 乾淨架構 + 正確基線（無資源洩漏） |
| **v1** 時空預約 + SIPP | TrafficControl 導 `TimeInterval/Headway` + `SafeIntervals()`；PathPlanning 換 `SippPathPlanner` | 車輛**錯時共用走廊**→首次吞吐躍升 |
| **v2** 優先級 + RHCR + 避免式預防 | 優先級 SIPP + PIBT；滾動時窗；`WouldCloseCycle` 接進 `TryReserve` | 高吞吐、延遲有界、**構造性 liveness** |
| **v3** CBS/CCBS + SIPPwRT | 壅塞 Zone 局部 CBS；連續時間 + 運動學 | 最高密度、平滑時間精確運動 |

---

## 9. 風險與不變條件

**風險**：R1（阻塞）NetDevPack fork 原始碼不在 workspace，須先取得；R2 即時預約狀態走**記憶體聚合**（`StateVersion` 樂觀並行），EF 僅快照/稽核；R3 v0 雖移植整路上鎖，但 `ReservationTable/ResourceLease/SpaceTimeCell` 一開始即以時間區間為基礎；R4 單一 `ReservationTable` 為爭用瓶頸（規模化按 zone 分片）；R6 迴圈節奏/重規劃確定性需明確定義避免 livelock。
