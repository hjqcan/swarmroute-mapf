...
| **v3** PIBT（壅塞區局部聯合步進） | 🟡 部分交付 | 執行層 **zone-local PIBT**（opt-in `UsePibt`，預設關閉）：偵測物理僵局簇 → 釋放其預約 → `PibtZoneResolver`（優先級繼承+回溯）逐 tick 聯合步進至清空，再回 prioritized-SIPP。**executor-anchored，不動凍結接縫**；CBS/CCBS、連續時間 SIPPwRT、`IJointStepPlanner`+預約表原子聯合提交+host-loop 接線 **→ v4** | 高密度物理僵局收斂（7×7/16 多 seed 由 DidNotConverge → Completed 或更多抵達，全程無碰撞）；範圍限 sim/執行層，未接 host loop |
| **v4** CBS/CCBS + 連續時間 SIPPwRT + PIBT host-seam | 下一階段 | 壅塞 Zone 局部 CBS/CCBS；SIPPwRT/TP-SIPPwRT 連續時間 + 運動學（Bezier v_max/a_max）；將 `PibtZoneResolver` 包進 `IJointStepPlanner` + `TryGrantJointStep` 原子聯合提交 → 封 cross-tick 健全性 + 帶 PIBT 進 autonomous host loop | 最高密度吞吐、平滑時間精確運動、host-loop 自治 |

> v2/v3 決策記錄（AA-SIPP(m)/h<w 否決；**v3 PIBT 採 executor-anchored、僅 sim/執行層**；CBS/CCBS + SIPPwRT + PIBT host-seam → v4；TrafficControl 讓路為 ordinal-id 非真 priority；WouldCloseCycle 在執行層 sim 中 inert）見 DDD 開發計劃 §「v2/v3 決策記錄」。
