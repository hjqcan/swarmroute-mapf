...
| **v3** PIBT + 局部 CBS（壅塞區聯合求解） | 🟡 部分交付 | 執行層 **zone-local PIBT**（opt-in `UsePibt`）與 **局部 CBS**（opt-in `UseCbs`，`CbsLocalSolver` 在 `PathPlanning.Domain`、重用 `SippPathPlanner` 當低階、經 `IFleetCoordinationCycle.PlanClusterAsync` 觸發+原子預約）：共用 `StuckClusterDetector` 偵測物理僵局簇 → 釋放預約 → PIBT 逐 tick 聯合步進 或 CBS 聯合求解 → 回 prioritized-SIPP。兩者是互斥的同層 cluster owner（同一物理僵局簇不可雙開）；CBS 只允許 SIPP/schedule-faithful，並沿用 RHCR horizon 與 executor physical blockers。皆預設關、executor-triggered、**不動凍結接縫**。連續時間 SIPPwRT/CCBS、`IJointStepPlanner`+預約表原子聯合提交+host-loop 接線 **→ v4** | 高密度物理僵局收斂（多 seed 由 DidNotConverge → Completed/更多抵達，全程無碰撞）；CBS 為完整局部解法（單測證明攻下 PIBT 攻不下的靜態案例），動態迴圈破部分 seed 但**不主宰 PIBT**；範圍限 sim/執行層 |
| **v4** 連續時間 SIPPwRT/CCBS + PIBT host-seam | 下一階段 | SIPPwRT/TP-SIPPwRT 連續時間 + 運動學（Bezier v_max/a_max）+ 把離散 `CbsLocalSolver` 升級成連續 CCBS（衝突=區間重疊、約束=運動依賴 unsafe-interval）；將 `PibtZoneResolver`/CBS 包進 `IJointStepPlanner` + `TryGrantJointStep` 原子聯合提交 → 封 cross-tick 健全性 + 帶 PIBT/CBS 進 autonomous host loop | 最高密度吞吐、平滑時間精確運動、host-loop 自治 |

> v2/v3 決策記錄（AA-SIPP(m)/h<w 否決；**v3 PIBT + 局部 CBS 採 executor-anchored、僅 sim/執行層**；CBS 重用 SIPP 低階、動態迴圈不主宰 PIBT；連續時間 SIPPwRT/CCBS + PIBT host-seam → v4；TrafficControl 讓路為 ordinal-id 非真 priority；WouldCloseCycle 在執行層 sim 中 inert）見 DDD 開發計劃 §「v2/v3 決策記錄」。
